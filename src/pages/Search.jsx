...
export default function Search() {
  ...
  const eksekusiPencarian = async (isLoadMore) => {
  ...
  useEffect(() => {
    if (page > 1) {
      eksekusiPencarian(true);
    }
  }, [page]);

  const handleCariFilm = (event) => {
    event.preventDefault();
    setPage(1);
    eksekusiPencarian(false);
  };
  ...
  return (
    <div className="w-full">
      <h1 className="text-3xl font-bold text-emerald-400 mb-8 border-l-4 border-emerald-400 pl-4">
        Cari judul Film{" "}
        <FontAwesomeIcon icon={faSearch} className="text-slate-400 ml-2" />
      </h1>

      <form
        onSubmit={handleCariFilm}
        className="max-w-2xl mx-auto mb-12 flex gap-2"
      >
        <input
          ref={inputRef}
          type="text"
      ...
      {loading && page === 1 ? (
        <div className="text-center text-slate-400 mt-20">
          <FontAwesomeIcon
            icon={faSpinner}
            spin
            className="text-4xl mb-4 text-emerald-400"
          />
        ...
        <>
          {sudahDicari && hasilPencarian.length === 0 && (
            <div className="text-center text-slate-400 mt-20">
              <p className="text-xl">
                Yah, film {kataKunci} tidak ditemukan{" "}
                <FontAwesomeIcon
                  icon={faCircleXmark}
          ...
          {hasilPencarian.length > 0 && (
            <div className="mb-8 mt-12 flex justify-center">
              ...
              >
                {loading ? "Menambahkan film" : "Muat Lebih banyak film"}
              </button>
            </div>
          )}
        </>
